...
def test_default_block_toc_contains_block_headers_only(mock_header_internal_id):
    ...
    assert block_toc_node.plain_entries[0] == block_header_node
...
def test_block_parenthood_labels():
    ...
    block_node = parser.nodes[0]
    label_title_nodes = block_node.labels["title"]
    label_role_nodes = block_node.labels["role"]

    # Each label must be a child of the
    # block it has been assigned to.
    check_parent(block_node, label_title_nodes)
    check_parent(block_node, label_role_nodes)
