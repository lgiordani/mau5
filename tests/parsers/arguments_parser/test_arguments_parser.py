...
from mau.test_helpers import parser_runner_factory, generate_context
...
def test_single_unnamed_argument_no_spaces():
    ...
    expected_node = Node(
        content=UnnamedArgumentNodeContent("value1"),
...
def test_single_unnamed_argument_with_spaces():
    ...
    expected_node = Node(
        content=UnnamedArgumentNodeContent("value with spaces"),
        info=NodeInfo(context=generate_context(0, 0)),
    )
...
def test_multiple_unnamed_arguments_no_spaces():
    ...
    assert runner(source).nodes == [
        ...
        Node(
            content=UnnamedArgumentNodeContent("value2"),
...
def test_multiple_unnamed_arguments_with_spaces():
    ...
    assert runner(source).nodes == [
        Node(
            content=UnnamedArgumentNodeContent("value1 with spaces"),
            info=NodeInfo(context=generate_context(0, 0)),
        ),
        Node(
            content=UnnamedArgumentNodeContent("value2 with more spaces"),
...
def test_single_unnamed_argument_comma_is_ignored_between_quotes():
    ...
    assert runner(source).nodes == [
        Node(
            content=UnnamedArgumentNodeContent("value1,value2"),
            info=NodeInfo(context=generate_context(0, 1)),
        )
    ]
...
def test_single_unnamed_argument_with_non_delimiting_quotes():
    ...
    assert runner(source).nodes == [
        Node(
            content=UnnamedArgumentNodeContent(
                'value "with quotes"',
            ),
            info=NodeInfo(context=generate_context(0, 0)),
        )
...
def test_single_named_argument():
    ...
    assert runner(source).nodes == [
        Node(
            content=NamedArgumentNodeContent("name", "value1"),
            info=NodeInfo(context=generate_context(0, 0)),
        ),
    ]
...
def test_single_named_argument_with_spaces():
    ...
    assert runner(source).nodes == [
        Node(
            content=NamedArgumentNodeContent("name", "value with spaces"),
            info=NodeInfo(context=generate_context(0, 0)),
        ),
    ]
...
def test_multiple_named_arguments():
    ...
    assert runner(source).nodes == [
        Node(
            content=NamedArgumentNodeContent("name1", "value1"),
            info=NodeInfo(context=generate_context(0, 0)),
        ),
        Node(
            content=NamedArgumentNodeContent("name2", "value2"),
...
def test_multiple_named_arguments_with_spaces():
    ...
    assert runner(source).nodes == [
        Node(
            content=NamedArgumentNodeContent("name1", "value1 with spaces"),
            info=NodeInfo(context=generate_context(0, 0)),
        ),
        Node(
            content=NamedArgumentNodeContent("name2", "value2 with spaces"),
...
def test_single_named_argument_with_quotes_no_spaces():
    ...
    assert runner(source).nodes == [
        Node(
            content=NamedArgumentNodeContent(
                "name",
                "value1",
            ),
            info=NodeInfo(context=generate_context(0, 0)),
...
def test_multiple_named_arguments_with_quotes_with_spaces():
    ...
    assert runner(source).nodes == [
        ...
        Node(
            content=NamedArgumentNodeContent("name2", "value2 with more spaces"),
...
def test_unnamed_and_named_arguments():
    ...
    assert runner(source).nodes == [
        ...
        Node(
            content=NamedArgumentNodeContent("name", "value2"),
