...
from mau.nodes.node import Node, NodeInfo, ValueNodeData
...
class ArgumentsParser(BaseParser):
    ...
    def __init__(
        ...
        self.unnamed_argument_nodes: list[Node[ValueNodeData]] = []

        # This is the list of named argument nodes.
        self.named_argument_nodes: dict[str, Node[ValueNodeData]] = {}

        # This is the list of tag nodes.
        self.tag_nodes: list[Node[ValueNodeData]] = []

        # This is the subtype node.
        self.subtype: Node[ValueNodeData] | None = None
    ...
    def _process_named_argument(self):
        ...
        node = Node(
            ...
            data=ValueNodeData(token.value.strip()),
    ...
    def _process_unnamed_argument(self):
        ...
        node = Node(
            info=NodeInfo(context=token.context),
            data=ValueNodeData(token.value),
            parent=self.parent_node,
        )
    ...
    def _isolate_tags_and_subtype(self):
        ...
        for i in self.unnamed_argument_nodes:
            ...
            if not i.data.value.startswith("#"):  # type: ignore[attr-defined]
                continue

            # Remove the initial `#`.
            i.data.value = i.data.value[1:]  # type: ignore[attr-defined]
        ...
        if self.subtype:
            ...
            subtype_replacement = subtype_replacements.get(
                self.subtype.data.value,  # type:ignore[attr-defined]
                {},
            )
    ...
    def arguments(self):
        return Arguments(
            unnamed_args=[node.data.value for node in self.unnamed_argument_nodes],  # type: ignore[attr-defined]
            named_args={
                key: node.data.value  # type: ignore[attr-defined]
                for key, node in self.named_argument_nodes.items()
            },
            tags=[node.data.value for node in self.tag_nodes],  # type: ignore[attr-defined]
            subtype=self.subtype.data.value if self.subtype else None,  # type: ignore[attr-defined]
        )
